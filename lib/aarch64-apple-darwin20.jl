...
function aws_wrapped_cf_allocator_new(allocator)
    ccall((:aws_wrapped_cf_allocator_new, libaws_c_common), CFAllocatorRef, (Ptr{aws_allocator},), allocator)
end
...
function aws_wrapped_cf_allocator_destroy(allocator)
    ccall((:aws_wrapped_cf_allocator_destroy, libaws_c_common), Cvoid, (CFAllocatorRef,), allocator)
end
...
function Base.getproperty(x::Ptr{aws_task}, f::Symbol)
    ...
    f === :abi_extension && return Ptr{__JL_Ctag_369}(x + 56)
    return getfield(x, f)
end
