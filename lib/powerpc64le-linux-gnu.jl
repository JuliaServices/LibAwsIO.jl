...
function Base.getproperty(x::Ptr{aws_socket}, f::Symbol)
    ...
    f === :io_handle && return Ptr{aws_io_handle}(x + 288)
    f === :event_loop && return Ptr{Ptr{aws_event_loop}}(x + 312)
    f === :handler && return Ptr{Ptr{aws_channel_handler}}(x + 320)
    f === :state && return Ptr{Cint}(x + 328)
    f === :readable_fn && return Ptr{Ptr{aws_socket_on_readable_fn}}(x + 336)
    f === :readable_user_data && return Ptr{Ptr{Cvoid}}(x + 344)
    f === :connection_result_fn && return Ptr{Ptr{aws_socket_on_connection_result_fn}}(x + 352)
    f === :accept_result_fn && return Ptr{Ptr{aws_socket_on_accept_result_fn}}(x + 360)
    f === :connect_accept_user_data && return Ptr{Ptr{Cvoid}}(x + 368)
    f === :impl && return Ptr{Ptr{Cvoid}}(x + 376)
    return getfield(x, f)
end
...
@cenum aws_tls_cipher_pref::UInt32 begin
    ...
    AWS_IO_TLS_CIPHER_PREF_NON_PQ_DEFAULT = 11
