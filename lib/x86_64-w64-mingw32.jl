...
function Base.getproperty(x::Ptr{aws_socket}, f::Symbol)
    ...
    f === :event_loop && return Ptr{Ptr{aws_event_loop}}(x + 592)
    f === :handler && return Ptr{Ptr{aws_channel_handler}}(x + 600)
    f === :state && return Ptr{Cint}(x + 608)
    f === :readable_fn && return Ptr{Ptr{aws_socket_on_readable_fn}}(x + 616)
    f === :readable_user_data && return Ptr{Ptr{Cvoid}}(x + 624)
    f === :connection_result_fn && return Ptr{Ptr{aws_socket_on_connection_result_fn}}(x + 632)
    f === :accept_result_fn && return Ptr{Ptr{aws_socket_on_accept_result_fn}}(x + 640)
    f === :connect_accept_user_data && return Ptr{Ptr{Cvoid}}(x + 648)
    f === :impl && return Ptr{Ptr{Cvoid}}(x + 656)
    return getfield(x, f)
end
