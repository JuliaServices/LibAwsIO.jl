...
function Base.getproperty(x::Ptr{aws_socket}, f::Symbol)
    ...
    f === :io_handle && return Ptr{aws_io_handle}(x + 584)
    f === :event_loop && return Ptr{Ptr{aws_event_loop}}(x + 608)
    f === :handler && return Ptr{Ptr{aws_channel_handler}}(x + 616)
    f === :state && return Ptr{Cint}(x + 624)
    f === :readable_fn && return Ptr{Ptr{aws_socket_on_readable_fn}}(x + 632)
    f === :readable_user_data && return Ptr{Ptr{Cvoid}}(x + 640)
    f === :connection_result_fn && return Ptr{Ptr{aws_socket_on_connection_result_fn}}(x + 648)
    f === :accept_result_fn && return Ptr{Ptr{aws_socket_on_accept_result_fn}}(x + 656)
    f === :connect_accept_user_data && return Ptr{Ptr{Cvoid}}(x + 664)
    f === :impl && return Ptr{Ptr{Cvoid}}(x + 672)
    return getfield(x, f)
end
