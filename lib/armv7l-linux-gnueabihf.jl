...
function Base.getproperty(x::Ptr{aws_socket}, f::Symbol)
    ...
    f === :io_handle && return Ptr{aws_io_handle}(x + 276)
    f === :event_loop && return Ptr{Ptr{aws_event_loop}}(x + 288)
    f === :handler && return Ptr{Ptr{aws_channel_handler}}(x + 292)
    f === :state && return Ptr{Cint}(x + 296)
    f === :readable_fn && return Ptr{Ptr{aws_socket_on_readable_fn}}(x + 300)
    f === :readable_user_data && return Ptr{Ptr{Cvoid}}(x + 304)
    f === :connection_result_fn && return Ptr{Ptr{aws_socket_on_connection_result_fn}}(x + 308)
    f === :accept_result_fn && return Ptr{Ptr{aws_socket_on_accept_result_fn}}(x + 312)
    f === :connect_accept_user_data && return Ptr{Ptr{Cvoid}}(x + 316)
    f === :impl && return Ptr{Ptr{Cvoid}}(x + 320)
    return getfield(x, f)
end
