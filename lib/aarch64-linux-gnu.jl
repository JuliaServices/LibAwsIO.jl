...
struct __JL_Ctag_719
    data::NTuple{8, UInt8}
end

function Base.getproperty(x::Ptr{__JL_Ctag_719}, f::Symbol)
    f === :scheduled && return Ptr{Bool}(x + 0)
    f === :reserved && return Ptr{Csize_t}(x + 0)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_719, f::Symbol)
    r = Ref{__JL_Ctag_719}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_719}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_719}, f::Symbol, v)
...
function Base.getproperty(x::Ptr{aws_io_handle}, f::Symbol)
    f === :data && return Ptr{__JL_Ctag_731}(x + 0)
...
function Base.getproperty(x::Ptr{aws_async_input_stream_tester}, f::Symbol)
    ...
    f === :synced_data && return Ptr{__JL_Ctag_733}(x + 184)
